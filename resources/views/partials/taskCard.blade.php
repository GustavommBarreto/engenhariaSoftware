...
<div class="bg-white rounded-lg shadow-sm hover:shadow-md transition p-3 border-l-4 {{ $priorityColor }} space-y-2">
    ...
    <a href="{{ route('tasks.show', $task) }}" class="block">
        <div class="flex justify-between items-start">
            ...
            @if($task->due_date && $task->due_date->isPast() && $task->status != 'DONE')
                <span class="text-xs text-red-500 font-semibold ml-2">ATRASADA</span>
            @endif
        </div>
    </a>
    ...
    <div class="flex justify-between items-center text-xs text-gray-500">
        ...
        <div class="flex items-center space-x-2">
            @if($task->assignee)
                <span title="Responsável: {{ $task->assignee->name }}">
            ...
            @if($task->due_date)
                <span class="flex items-center" title="Prazo: {{ $task->due_date->format('d/m/Y') }}">
